...
#define FUSION_MAX_OPS_LIMIT 2048
#define FUSION_MAX_LEVELS 64
#define FUSION_MAX_LOCS 4096
...
typedef struct {
    ...
    uint32_t op_code;         // specific op (EXP, ADD, etc.)
    uint32_t map_id;
...
int adamah_init_ex(uint64_t hot_bytes, uint64_t cold_bytes) {
    ...
    // Estimate compute units from workgroup limits (rough heuristic)
    // More VRAM usually means more compute units
    if (gpu_caps.vram_bytes >= 16ull * 1024 * 1024 * 1024) {
        gpu_caps.max_compute_units = 128;  // High-end GPU (16GB+)
    } else if (gpu_caps.vram_bytes >= 8ull * 1024 * 1024 * 1024) {
        gpu_caps.max_compute_units = 64;   // Mid-range GPU (8GB+)
    } else if (gpu_caps.vram_bytes >= 4ull * 1024 * 1024 * 1024) {
        gpu_caps.max_compute_units = 32;   // Entry GPU (4GB+)
    } else {
        gpu_caps.max_compute_units = 16;   // Integrated/low-end
    }
    
    // Set fusion queue size based on GPU capability
    // More powerful GPU = larger fusion queue = better batching
    // Limited by descriptor sets and memory
    gpu_caps.fusion_max_ops = gpu_caps.max_compute_units * 128;  // 16 ops per compute unit
    if (gpu_caps.fusion_max_ops > FUSION_MAX_OPS_LIMIT) {
        gpu_caps.fusion_max_ops = FUSION_MAX_OPS_LIMIT;
    }
    if (gpu_caps.fusion_max_ops < 128) {
        gpu_caps.fusion_max_ops = 128;  // Minimum
    }
    
    // Optimal batch size - try to saturate GPU without overwhelming it
    gpu_caps.optimal_batch_size = gpu_caps.max_compute_units * 4;
    if (gpu_caps.optimal_batch_size < 32) gpu_caps.optimal_batch_size = 32;
    if (gpu_caps.optimal_batch_size > 512) gpu_caps.optimal_batch_size = 512;
    
    fprintf(stderr, "[ADAMAH] GPU Caps: VRAM=%.1fGB, fusion_max_ops=%u, optimal_batch=%u\n",
...
int adamah_init(void) {
    // First do minimal init to get GPU caps
    // We need to query the device before we can set pool sizes
    
    // Use default pools initially, but we could make this smarter
    // by doing a two-phase init: query device, then allocate pools
    uint64_t hot = HOT_POOL_BYTES_DEFAULT;
    uint64_t cold = COLD_POOL_BYTES_DEFAULT;
    
    // After adamah_init_ex runs, gpu_caps will be populated
    // For now use defaults, but log what we'd recommend
    int ret = adamah_init_ex(hot, cold);
    
    if (ret == ADAMAH_OK && gpu_caps.vram_bytes > 0) {
        // Log recommended sizes based on VRAM
        uint64_t recommended_hot = gpu_caps.vram_bytes / 4;   // 25% of VRAM for hot
        uint64_t recommended_cold = gpu_caps.vram_bytes / 8;  // 12.5% of VRAM for cold
        
        // Cap at reasonable maximums
        if (recommended_hot > 4ull * 1024 * 1024 * 1024) recommended_hot = 4ull * 1024 * 1024 * 1024;
        if (recommended_cold > 2ull * 1024 * 1024 * 1024) recommended_cold = 2ull * 1024 * 1024 * 1024;
        
        fprintf(stderr, "[ADAMAH] Pool sizes: hot=%.0fMB, cold=%.0fMB (recommended: hot=%.0fMB, cold=%.0fMB)\n",
                (double)hot / (1024.0 * 1024.0),
                (double)cold / (1024.0 * 1024.0),
                (double)recommended_hot / (1024.0 * 1024.0),
                (double)recommended_cold / (1024.0 * 1024.0));
    }
...
static int init_pipelines(void) {
    ...
    // 3. ./adamah/shaders (new structure, running from repo root)
    // 4. ./shaders (legacy)
    // 5. ./src/adamah/shaders (old structure)
    // 6. System path
    const char* paths[] = { 
#ifdef SHADER_PATH
        SHADER_PATH,
#endif
        lib_shaders[0] ? lib_shaders : NULL,
        "./adamah/shaders",
        "./shaders", 
        "./src/adamah/shaders", 
    ...
    for (int i = 0; paths[i] != NULL; i++) {
        if (paths[i] == NULL) continue;  // Skip NULL entries
        char test[600];
...
int adamah_fusion_is_enabled(void) {
...
static int fusion_calc_level(uint32_t locs_src, uint32_t locs_src2) {
    ...
    if (locs_src2 != 0 && locs_src2 < FUSION_MAX_LOCS && fusion.loc_write_level[locs_src2] >= 0) {
        int lvl2 = fusion.loc_write_level[locs_src2] + 1;
        if (lvl2 > level) level = lvl2;
    }
...
static void fusion_auto_flush_if_needed(int new_level) {
    (void)new_level;  // Level doesn't trigger flush - only used for ordering during execution
    
    // Flush only if queue is full (use dynamic limit based on GPU)
    uint32_t max_ops = gpu_caps.fusion_max_ops > 0 ? gpu_caps.fusion_max_ops : 512;
...
static int fusion_queue_op1(uint32_t map_id, uint32_t op, uint32_t locs_src, uint32_t locs_dst, uint32_t n) {
...
static int fusion_queue_op2(uint32_t map_id, uint32_t op, 
                            uint32_t locs_a, uint32_t locs_b, uint32_t locs_dst, uint32_t n) {
...
static int fusion_queue_matmul(uint32_t map_id, uint32_t locs_a, uint32_t locs_b, uint32_t locs_c,
...
static int fusion_queue_reduce(uint32_t map_id, uint32_t op,
                               uint32_t locs_src, uint32_t locs_dst, uint32_t n) {
    int level = fusion_calc_level(locs_src, 0);
    fusion_auto_flush_if_needed(level);
...
static int fusion_queue_softmax(uint32_t map_id, uint32_t locs_src, uint32_t locs_dst, 
                                uint32_t dim, uint32_t n_rows) {
    int level = fusion_calc_level(locs_src, 0);
    fusion_auto_flush_if_needed(level);
...
static int fusion_queue_layernorm(uint32_t map_id, uint32_t locs_src, uint32_t locs_dst,
                                  uint32_t locs_gamma, uint32_t locs_beta,
                                  uint32_t dim, uint32_t n_rows, float eps) {
...
static int fusion_queue_broadcast(uint32_t map_id, uint32_t op,
                                  uint32_t locs_a, uint32_t locs_scalar, uint32_t locs_dst, uint32_t n) {
...
static int exec_op1_internal(uint32_t map_id, uint32_t op, uint32_t locs_src, uint32_t locs_dst, uint32_t n);
static int exec_op2_internal(uint32_t map_id, uint32_t op, uint32_t locs_a, uint32_t locs_b, uint32_t locs_dst, uint32_t n);
static int exec_matmul_internal(uint32_t map_id, uint32_t locs_a, uint32_t locs_b, uint32_t locs_c, uint32_t M, uint32_t K, uint32_t N, uint32_t n_ops);
static int exec_reduce_internal(uint32_t map_id, uint32_t op, uint32_t locs_src, uint32_t locs_dst, uint32_t n);
static int exec_softmax_internal(uint32_t map_id, uint32_t locs_src, uint32_t locs_dst, uint32_t dim, uint32_t n_rows);
static int exec_layernorm_internal(uint32_t map_id, uint32_t locs_src, uint32_t locs_dst, uint32_t locs_gamma, uint32_t locs_beta, uint32_t dim, uint32_t n_rows, float eps);
static int exec_broadcast_internal(uint32_t map_id, uint32_t op, uint32_t locs_a, uint32_t locs_scalar, uint32_t locs_dst, uint32_t n);

// Execute all queued operations, grouped by level
int adamah_fusion_flush(void) {
    if (fusion.n_ops == 0) return ADAMAH_OK;
    ...
    for (int level = 0; level <= fusion.max_level; level++) {
        ...
        for (int i = 0; i < fusion.n_ops; i++) {
            ...
            if (fop->level != level) continue;
            
            int ret = ADAMAH_OK;
            switch (fop->op_type) {
                case FUSE_OP_UNARY:
                    ret = exec_op1_internal(fop->map_id, fop->op_code, fop->locs_src, fop->locs_dst, fop->n);
                    break;
                case FUSE_OP_BINARY:
                    ret = exec_op2_internal(fop->map_id, fop->op_code, fop->locs_src, fop->locs_src2, fop->locs_dst, fop->n);
                    break;
                case FUSE_OP_MATMUL:
                    ret = exec_matmul_internal(fop->map_id, fop->locs_extra[0], fop->locs_extra[1], fop->locs_extra[2], fop->M, fop->K, fop->N, fop->n);
                    break;
                case FUSE_OP_REDUCE:
                    ret = exec_reduce_internal(fop->map_id, fop->op_code, fop->locs_src, fop->locs_dst, fop->n);
                    break;
                case FUSE_OP_SOFTMAX:
                    ret = exec_softmax_internal(fop->map_id, fop->locs_src, fop->locs_dst, fop->M, fop->n);
                    break;
                case FUSE_OP_LAYERNORM:
                    ret = exec_layernorm_internal(fop->map_id, fop->locs_src, fop->locs_dst, fop->locs_extra[0], fop->locs_extra[1], fop->M, fop->n, fop->eps);
                    break;
                case FUSE_OP_BROADCAST:
                    ret = exec_broadcast_internal(fop->map_id, fop->op_code, fop->locs_src, fop->locs_src2, fop->locs_dst, fop->n);
                    break;
            }
            if (ret != ADAMAH_OK) result = ret;
        }
        
        // Barrier between levels (if not the last level)
        // The barrier is already added by cmd_barrier_after_dispatch() in each exec_*_internal
    }
...
void adamah_fusion_stats(int* n_ops, int* n_levels) {
    if (n_ops) *n_ops = fusion.n_ops;
    if (n_levels) *n_levels = fusion.max_level + 1;
}
...
static int exec_op1_internal(uint32_t map_id, uint32_t op, uint32_t locs_src_handle, uint32_t locs_dst_handle, uint32_t n) {
    if (map_id >= MAX_MAPS || !ctx.maps[map_id].active) return ADAMAH_ERR_INVALID;
...
int map_op2_dev(uint32_t map_id, uint32_t op,
                uint32_t locs_a_handle, uint32_t locs_b_handle, uint32_t locs_dst_handle, uint32_t n) {
    if (fusion.enabled) {
        return fusion_queue_op2(map_id, op, locs_a_handle, locs_b_handle, locs_dst_handle, n);
    }
    return exec_op2_internal(map_id, op, locs_a_handle, locs_b_handle, locs_dst_handle, n);
}
...
int map_matmul_dev(uint32_t map_id,
                   ...
                   uint32_t M, uint32_t K, uint32_t N, uint32_t n_ops) {
    if (fusion.enabled) {
        return fusion_queue_matmul(map_id, locs_a_handle, locs_b_handle, locs_c_handle, M, K, N, n_ops);
    }
    return exec_matmul_internal(map_id, locs_a_handle, locs_b_handle, locs_c_handle, M, K, N, n_ops);
}

// Internal execution - bypasses fusion
static int exec_matmul_internal(uint32_t map_id,
                   uint32_t locs_a_handle, uint32_t locs_b_handle, uint32_t locs_c_handle,
                   uint32_t M, uint32_t K, uint32_t N, uint32_t n_ops) {
...
int map_broadcast_dev(uint32_t map_id, uint32_t op,
                      ...
                      uint32_t n) {
    if (fusion.enabled) {
        return fusion_queue_broadcast(map_id, op, locs_src_handle, locs_scalar_handle, locs_dst_handle, n);
    }
    return exec_broadcast_internal(map_id, op, locs_src_handle, locs_scalar_handle, locs_dst_handle, n);
}
...
// Internal execution - bypasses fusion
static int exec_softmax_internal(uint32_t map_id,
...
int map_softmax_dev(uint32_t map_id,
                    ...
                    uint32_t n_rows, uint32_t row_size) {
    if (fusion.enabled) {
        return fusion_queue_softmax(map_id, locs_src_handle, locs_dst_handle, row_size, n_rows);
    }
    return exec_softmax_internal(map_id, locs_src_handle, locs_dst_handle, row_size, n_rows);
}
...
// Internal execution - bypasses fusion
static int exec_layernorm_internal(uint32_t map_id,
                      uint32_t locs_src_handle, uint32_t locs_dst_handle,
...
int map_layernorm_dev(uint32_t map_id,
                      ...
                      uint32_t n_rows, uint32_t dim, float eps) {
    if (fusion.enabled) {
        return fusion_queue_layernorm(map_id, locs_src_handle, locs_dst_handle, locs_gamma_handle, locs_beta_handle, dim, n_rows, eps);
    }
    return exec_layernorm_internal(map_id, locs_src_handle, locs_dst_handle, locs_gamma_handle, locs_beta_handle, dim, n_rows, eps);
}
