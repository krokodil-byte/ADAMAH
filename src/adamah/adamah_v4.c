...
static int init_pipelines(void) {
    // Find shader path
    const char* paths[] = { "./shaders", "./src/adamah/shaders", "/usr/share/adamah/shaders" };
    for (int i = 0; i < 3; i++) {
        char test[600];
        snprintf(test, sizeof(test), "%s/map_op1.spv", paths[i]);
        FILE* f = fopen(test, "rb");
        if (f) { fclose(f); strncpy(ctx.shader_path, paths[i], 511); break; }
    }
    if (!ctx.shader_path[0]) return ADAMAH_ERR_INVALID;
    
    // Push constant size: op, n_locs, pack_size = 12 bytes
